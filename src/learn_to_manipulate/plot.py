        ...
        for file_path_index in range(len(files)):
            ...
            for episode_index in range(num_episodes):
                ...
                if "teleop" in episode.controller_type:
                    total_cost = total_cost + human_demo_cost
                elif not episode.result:
                    total_cost = total_cost + failure_cost
...
def plot_limited_demos(folders, method_names, max_episodes = 500):
    plt.close("all")
    plt.rcParams.update({'font.size': 14})
    ...
    for folder_ind in range(len(folders)):
        ...
        for file_path_index in range(len(files)):
            ...
            successes = 0.0
            sliding_window_mean_success_rate = []
            success_list, demo_list = pickle.load(file)

            for episode_index in range(len(success_list)):
                window_successes = 0.0
                window_episodes = 0.0

                # loop through the window and tally success and failure
                for i in range((episode_index - window_half_width), (episode_index + window_half_width + 1)):
                    if (i < 0) or (i >= len(success_list)):
                        continue
...
def plot_baseline_success_rate(folders, method_names, max_episodes = 500):
    plt.close("all")
    plt.rcParams.update({'font.size': 14})
    ...
    for folder_ind in range(len(folders)):
        ...
        for file_path_index in range(len(files)):
            ...
            for episode_index in range(len(data_list)):
                ...
                if data_list[episode_index].controller_type != 'baseline':
                    continue
